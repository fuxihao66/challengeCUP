...
namespace pcl
{
    ...
    class Kinect2Grabber : public pcl::Grabber
    {
        public:
			...
			typedef void (signal_Kinect2_Overall)(const boost::shared_ptr<const pcl::PointCloud<pcl::PointXYZRGBA>>&, const boost::shared_ptr<myMath::myImage>&);
        ...
        protected:
			...
			boost::signals2::signal<signal_Kinect2_Overall>* signal_Overall;
			...
			boost::shared_ptr<myMath::myImage> convertRGBADepthToImage(RGBQUAD* colorBuffer, UINT16* depthBuffer);
        ...
        , signal_PointXYZRGBA( nullptr )
    {
		...
		signal_Overall = createSignal< signal_Kinect2_Overall>();
    }
    ...
    pcl::Kinect2Grabber::~Kinect2Grabber() throw()
    {
		...
		disconnect_all_slots<signal_Kinect2_Overall>();
    ...
    void pcl::Kinect2Grabber::threadFunction()
    {
        while( !quit ){
			...
			if (signal_Overall->num_slots() > 0) {
				signal_Overall->operator()(convertRGBADepthToPointXYZRGBA(&colorBuffer[0], &depthBuffer[0]), convertRGBADepthToImage(&colorBuffer[0], &depthBuffer[0]));
			}
        }
    }
	...
	// 输出1920x1080图片
	//boost::shared_ptr<myMath::myImage> pcl::Kinect2Grabber::convertRGBADepthToImage(RGBQUAD* colorBuffer, UINT16* depthBuffer)
	//{
	...
	//	img->resize(colorWidth, colorHeight);

	//	for (size_t y = 0; y < colorHeight; y++) {
	//		for (size_t x = 0; x < colorWidth; x++) {
	//			RGBQUAD color = colorBuffer[y*colorWidth + x];
	//			img->points[y*colorWidth + x].r = color.rgbRed;
	//			img->points[y*colorWidth + x].g = color.rgbGreen;
	//			img->points[y*colorWidth + x].b = color.rgbBlue;
	//			img->points[y*colorWidth + x].a = color.rgbReserved;
	...
	//				img->points[colorY*colorWidth + colorX].x = point.x;
	//				img->points[colorY*colorWidth + colorX].y = point.y;
	//				img->points[colorY*colorWidth + colorX].z = point.z;
	//			}
		...
		for (int y = 0; y < depthHeight; y++) {
			for (int x = 0; x < depthWidth; x++) {
				...
				if ((0 <= colorX) && (colorX < colorWidth) && (0 <= colorY) && (colorY < colorHeight)) {
					...
					img->points[y*width + x].copyFromPCLPoint(point);
				}
